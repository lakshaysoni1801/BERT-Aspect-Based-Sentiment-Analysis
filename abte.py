...
class ABTEModel ():
    ...
    def train(self, data, epochs, device, batch_size=32, lr=1e-5, load_model=None, lr_schedule=True):
        ...
        for epoch in range(epochs):
            ...
            # FIX: iterate the loader itself instead of calling next(iter(loader))
            # every step. The old code created a brand-new iterator (and reshuffled)
            # on each iteration and only ever consumed its first batch, so training
            # was really `n_batches` random draws with replacement, not one true
            # pass over the dataset. enumerate(loader) now walks the full epoch.
            for nb, (ids_tensors, tags_tensors, _, masks_tensors) in enumerate(loader):
                t0 = time.time()
